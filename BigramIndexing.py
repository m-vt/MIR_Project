...
dict = {" ": {}}
def make_bigram_index():
    with open('outfile', 'rb') as fp:
        itemlist = pickle.load(fp)
    itemlist = list(itemlist)


    def bigram_one_word(test_str):
        global dict
        main_word = test_str
        size = len(test_str)
        for i in range(size - 1):
            ...
            if dict.__contains__(bigram):
                dict.get(bigram).add(main_word)
            else:
                dict.update({bigram: {main_word}})
            test_str = test_str[1:]


    for i in range(len(itemlist)):
        # print(type(itemlist[i]))
        bigram_one_word(itemlist[i])
        if i == 100:
            break
